		...
		const char **idxs, int idxs_num, zbx_vector_uint64_t *value_ids)
{
	...
	zbx_DBselect_uint64(sql, profileids);
		...
		const zbx_vector_uint64_t *elementids)
{
	...
	zbx_DBselect_uint64(sql, selementids);
...
static int	validate_linked_templates(const zbx_vector_uint64_t *templateids, char *error, size_t max_error_len)
{
	...
	if (SUCCEED == ret && 1 < templateids->values_num)
	{
		...
		zbx_DBselect_uint64(sql, &graphids);
		...
		char *error, size_t max_error_len)
{
	...
	tresult = zbx_DBselect("%s", sql);

	while (NULL != (trow = zbx_DBfetch(tresult)))
	{
		...
		sresult = zbx_DBselectN(sql, 1);

		if (NULL != zbx_DBfetch(sresult))
		{
			ret = FAIL;
			zbx_snprintf(error, max_error_len,
...
static int	validate_host(zbx_uint64_t hostid, zbx_vector_uint64_t *templateids, char *error, size_t max_error_len)
{
	...
	if (SUCCEED == ret)
	{
		...
		tresult = zbx_DBselectN(sql, 1);

		if (NULL != (trow = zbx_DBfetch(tresult)))
		{
			ret = FAIL;
			zbx_snprintf(error, max_error_len,
	...
	if (SUCCEED == ret)
	{
		...
		while (SUCCEED == ret && NULL != (trow = zbx_DBfetch(tresult)))
		{
			type = (unsigned char)atoi(trow[0]);
...
static void	DBdelete_action_conditions(int conditiontype, zbx_uint64_t elementid)
{
	...
	/* disable actions */
	result = zbx_DBselect("select actionid,conditionid from conditions where conditiontype=%d and"
...
static void	DBgroup_prototypes_delete(const zbx_vector_uint64_t *del_group_prototypeids)
{
	...
	zbx_DBselect_uint64(sql, &groupids);
		...
		const zbx_vector_str_t *host_prototype_names)
{
	...
	zbx_DBselect_uint64(sql, &group_prototype_ids);
...
static int	DBis_regular_host(zbx_uint64_t hostid)
{
	...
	result = zbx_DBselect("select flags from hosts where hostid=" ZBX_FS_UI64, hostid);
		...
		while (NULL != (row = zbx_DBfetch(result)))
		{
			...
			for (i = 0; i < host_prototypes->values_num; i++)
			{
				...
				if (host_prototype->itemid == itemid && 0 == strcmp(host_prototype->host, row[2]))
				{
					...
					if (SUCCEED == zbx_DBis_null(row[8]))
...
void	DBdelete_hosts(const zbx_vector_uint64_t *hostids, const zbx_vector_str_t *hostnames)
{
	...
	zbx_DBselect_uint64(sql, &httptestids);
		...
		const char *ip, const char *dns, unsigned short port, zbx_conn_flags_t flags)
{
	...
	while (NULL != (row = zbx_DBfetch(result)))
	{
		...
		if (1 == db_main)
		{
			...
			if (0 != update_alloc)
			{
				zbx_DBexecute("update interface set%s where interfaceid=" ZBX_FS_UI64, update,
						interfaceid);
				zbx_free(update);
			}
			break;
	...
	zbx_DBexecute("insert into interface"
		...
		const zbx_uint64_t hostid)
{
	...
	if (NULL == row)
	{
		zbx_DBexecute("insert into interface_snmp"
...
static void	DBdelete_groups_validate(zbx_vector_uint64_t *groupids)
{
	...
	zbx_DBselect_uint64(sql, &hostids);
...
void	DBset_host_inventory(zbx_uint64_t hostid, int inventory_mode)
{
	...
	result = zbx_DBselect("select inventory_mode from host_inventory where hostid=" ZBX_FS_UI64, hostid);
	...
	{
		zbx_DBexecute("update host_inventory set inventory_mode=%d where hostid=" ZBX_FS_UI64, inventory_mode,
				hostid);
