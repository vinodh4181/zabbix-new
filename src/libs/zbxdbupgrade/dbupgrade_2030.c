...
static int	DBpatch_2030024(void)
{
	...
	if (NULL == (result = zbx_DBselect("select itemid,filter from items where filter<>'' and flags=1")))
		return FAIL;
	...
	while (NULL != (row = zbx_DBfetch(result)))
	{
		...
		*value++ = '\0';
		...
		rc = zbx_DBexecute("insert into item_condition"
...
{
	if (ZBX_DB_OK > zbx_DBexecute("update items set formula='' where flags=%d", ZBX_FLAG_DISCOVERY_RULE))
		return FAIL;
...
static int	DBpatch_2030029(void)
{
	...
	if (ZBX_DB_OK > zbx_DBexecute("update screens_items set sort_triggers=7 where sort_triggers=9"))
		return FAIL;
...
static int	DBpatch_2030030(void)
{
	...
	if (ZBX_DB_OK > zbx_DBexecute("update screens_items set sort_triggers=8 where sort_triggers=10"))
		return FAIL;
...
static int	DBpatch_2030031(void)
{
	...
	if (ZBX_DB_OK > zbx_DBexecute("update conditions set value='' where conditiontype=16"))
		return FAIL;
		...
		int field_length)
{
	...
	/* 1 - ZBX_NODE_LOCAL */
	if (NULL == (result = zbx_DBselect("select nodeid from nodes where nodetype=1")))
		return FAIL;
	...
	while (NULL != (row = zbx_DBfetch(result)))
	{
		...
		if (ZBX_DB_OK > zbx_DBexecute("update " ZBX_FS_SQL_NAME " set " ZBX_FS_SQL_NAME "='%s'"
...
static int	check_data_uniqueness(const char *table_name, const char *field_name)
{
	...
	if (NULL == (result = zbx_DBselect("select %s from %s group by %s having count(*)>1",
...
static int	DBpatch_2030068(void)
{
	if (0 != (program_type & ZBX_PROGRAM_TYPE_PROXY))
	{
		...
		if (ZBX_DB_OK > zbx_DBexecute("update drules set name=druleid"))
			return FAIL;
	}
...
static int	DBpatch_2030094(void)
{
	...
	result = zbx_DBselect("select triggerid,expression from triggers");
	...
	while (SUCCEED == ret && NULL != (row = zbx_DBfetch(result)))
	{
		...
		else if (0 != strcmp(row[1], expr))
		{
			...
			if (ZBX_DB_OK > zbx_DBexecute("update triggers set expression='%s' where triggerid=%s",
					expr_esc, row[0]))
			{
				ret = FAIL;
			}
...
static int	DBpatch_2030095(void)
{
	...
	result = zbx_DBselect("select itemid,params from items where type=%d", 15 /* ITEM_TYPE_CALCULATED */);
...
{
	if (ZBX_DB_OK <= zbx_DBexecute("update screens_items set colspan=1 where colspan=0"))
		return SUCCEED;
...
{
	if (ZBX_DB_OK <= zbx_DBexecute("update screens_items set rowspan=1 where rowspan=0"))
		return SUCCEED;
...
{
	if (ZBX_DB_OK > zbx_DBexecute("delete from profiles where idx='web.view.application'"))
		return FAIL;
