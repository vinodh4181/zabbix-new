...
static int	DBpatch_2050001(void)
{
	...
	if (NULL == (result = zbx_DBselect("select itemid,snmp_oid from items where flags=1 and type in (1,4,6)")))
		return FAIL;
	...
	while (NULL != (row = zbx_DBfetch(result)))
	{
		...
		else
		{
			...
			rc = zbx_DBexecute("update items set snmp_oid='%s' where itemid=%s", oid_esc, row[0]);
...
static int	DBpatch_2050011(void)
{
	...
	if (ZBX_DB_OK <= zbx_DBexecute("update items set trends=0 where value_type in (1,2,4)"))
		return SUCCEED;
...
static int	DBpatch_2050012(void)
{
	...
	while (SUCCEED == ret && NULL != (row = zbx_DBfetch(result)))
	{
		...
		result2 = zbx_DBselect("select null from items where hostid=%s and key_='%s'", row[0], key_esc);

		if (NULL == zbx_DBfetch(result2))
		{
			if (ZBX_DB_OK > zbx_DBexecute("update items set key_='%s' where itemid=%s", key_esc, row[1]))
				ret = FAIL;
		}
		else
		{
...
static int	DBpatch_2050063(void)
{
	...
	if (ZBX_DB_OK > zbx_DBexecute("update media_type"
...
static int	DBpatch_2050078(void)
{
	...
	if (ZBX_DB_OK > zbx_DBexecute("update sysmaps set userid=(select min(userid) from users where type=3)"))
		return FAIL;
...
{
	if (ZBX_DB_OK > zbx_DBexecute("update profiles"
...
static int	DBpatch_2050092(void)
{
	...
	if (NULL == (result = zbx_DBselect("select userid,url from users where url<>''")))
		return FAIL;
	...
	while (NULL != (row = (zbx_DBfetch(result))))
	{
		...
		rc = zbx_DBexecute("update users set url='%s' where userid=%s", url_esc, row[0]);
		zbx_free(url_esc);
...
{
	/* type=3 -> type=USER_TYPE_SUPER_ADMIN */
	if (ZBX_DB_OK > zbx_DBexecute("update screens"
...
static int	DBpatch_2050106(void)
{
	...
	if (ZBX_DB_OK > zbx_DBexecute("update proxy_history set flags=3 where flags=1"))
	{
		return FAIL;
	}
...
{
	/* type=3 -> type=USER_TYPE_SUPER_ADMIN */
	if (ZBX_DB_OK > zbx_DBexecute("update slideshows set userid=(select min(userid) from users where type=3)"))
		return FAIL;
...
static int	DBpatch_2050120(void)
{
	...
	if (ZBX_DB_OK <= zbx_DBexecute("update sysmaps set private=0"))
		return SUCCEED;
...
{
	/* private=0 -> PUBLIC_SHARING */
	if (ZBX_DB_OK <= zbx_DBexecute("update screens set private=0"))
		return SUCCEED;
...
{
	/* private=0 -> PUBLIC_SHARING */
	if (ZBX_DB_OK <= zbx_DBexecute("update slideshows set private=0"))
		return SUCCEED;
