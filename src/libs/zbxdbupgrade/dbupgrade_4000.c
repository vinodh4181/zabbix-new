...
static int	DBpatch_4000001(void)
{
	...
	result = zbx_DBselect("select actionid,def_shortdata,def_longdata,r_shortdata,r_longdata,ack_shortdata,"
...
static int	DBpatch_4000003(void)
{
	...
	/* 0 - EVENT_SOURCE_TRIGGERS */
	result = zbx_DBselect("select oc.operationid,oc.command"
