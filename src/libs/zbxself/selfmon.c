...
int	zbx_init_selfmon_collector(zbx_get_config_forks_f get_config_forks, char **error)
{
	...
	sz_total = zbx_monitor_get_size(units_num);
	...
	zbx_monitor_sync_init(&collector.sync, sm_sync_lock, sm_sync_unlock, (void *)&sm_lock);
	collector.monitor = zbx_monitor_create_ext(units_num, &collector.sync, __sm_shmem_malloc_func,
...
void	zbx_free_selfmon_collector(void)
{
	...
	zbx_monitor_destroy(collector.monitor);
...
void	zbx_update_selfmon_counter(const zbx_thread_info_t *info, unsigned char state)
{
	...
	zbx_monitor_update(collector.monitor, unit_index, state);
...
static void	collect_selfmon_stats(void)
{
	...
	zbx_monitor_collect(collector.monitor);
		...
		double *value)
{
	...
	if (SUCCEED != zbx_monitor_get_stat(collector.monitor, unit_index, unit_count, aggr_func, state, value, &error))
	{
	...
	for (proc_type = 0; proc_type < ZBX_PROCESS_TYPE_COUNT; proc_type++)
	{
		...
		for (proc_num = 0; proc_num < stats[proc_type].count; proc_num++)
		{
			...
			for (s = 0; s < ZBX_PROCESS_STATE_COUNT; s++)
				one_total += units[unit_index].counters[s];

			busy_counter = units[unit_index].counters[ZBX_PROCESS_STATE_BUSY];
			idle_counter = units[unit_index].counters[ZBX_PROCESS_STATE_IDLE];
