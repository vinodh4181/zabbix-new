...
static int	tm_execute_remote_command(zbx_uint64_t taskid, int clock, int ttl, int now, int config_timeout)
{
	...
	if (SUCCEED != (ret = zbx_script_execute(&script, &host, NULL, 0 == alertid ? &info : NULL,
			config_timeout, error, sizeof(error), NULL)))
	{
 ...
 *                                                                            *
 ******************************************************************************/
static int	tm_execute_data_json(int type, const char *data, char **info, int config_timeout)
{
	...
	switch (type)
	{
		case ZBX_TM_DATA_TYPE_TEST_ITEM:
			return zbx_trapper_item_test_run(&jp_data, 0, info, config_timeout);
		...
		int config_timeout)
{
	...
	switch (data_type = atoi(row[2]))
	{
		case ZBX_TM_DATA_TYPE_TEST_ITEM:
		case ZBX_TM_DATA_TYPE_DIAGINFO:
			ret = tm_execute_data_json(data_type, row[1], &info, config_timeout);
			break;
 ...
 *                                                                            *
 ******************************************************************************/
static int	tm_process_tasks(zbx_ipc_async_socket_t *rtc, int now, int config_timeout)
{
	DB_ROW			row;
	DB_RESULT		result;
	...
	while (NULL != (row = DBfetch(result)))
	{
		...
		switch (type)
		{
			...
			case ZBX_TM_TASK_DATA:
				if (SUCCEED == tm_execute_data(rtc, taskid, clock, ttl, now, config_timeout))
					processed_num++;
				break;
			default:
				THIS_SHOULD_NEVER_HAPPEN;
...
ZBX_THREAD_ENTRY(taskmanager_thread, args)
{
	...
	while (ZBX_IS_RUNNING())
	{
		...
		tasks_num = tm_process_tasks(&rtc, (int)sec1, taskmanager_args_in->config_timeout);
