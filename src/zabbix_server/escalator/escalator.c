...
static int	get_user_info(zbx_uint64_t userid, zbx_uint64_t *roleid, char **user_timezone)
{
	...
	*user_timezone = NULL;
	...
	result = zbx_DBselect("select r.type,u.roleid,u.timezone from users u,role r where u.roleid=r.roleid and"
		...
		const ZBX_DB_EVENT *event)
{
	...
	result = zbx_DBselect("%s order by tf.groupid", sql);
...
static int	check_db_parent_rule_tag_match(zbx_vector_uint64_t *parent_ids, zbx_vector_tags_t *tags)
{
	...
	result = zbx_DBselect("%s) limit 1", sql);
...
static void	zbx_db_cache_service_role(zbx_service_role_t *role)
{
	...
	result = zbx_DBselect("select name,roleid,value_int,value_str,value_serviceid,type from role_rule where roleid="
	...
	while (NULL != (row = zbx_DBfetch(result)))
	{
		...
		else if (ZBX_ROLE_RULE_TYPE_SERVICEID == type) /* services.read.id.<idx> / services.write.id.<idx>*/
		{
			...
			if (SUCCEED == zbx_DBis_null(value_serviceid))
				continue;
		...
		const char *user_timezone)
{
	...
	if (0 != mediatypeid)
	{
		result = zbx_DBselect("select mediatype_messageid,subject,message,mediatypeid from media_type_message"
...
static void	add_sentusers_ack_msg(ZBX_USER_MSG **user_msg, zbx_uint64_t actionid, zbx_uint64_t operationid,
		const ZBX_DB_EVENT *event, const ZBX_DB_EVENT *r_event, const ZBX_DB_ACKNOWLEDGE *ack,
		unsigned char evt_src, const char *default_timezone)
{
	DB_RESULT	result;
	DB_ROW		row;
	zbx_uint64_t	userid;
...
static void	get_operation_groupids(zbx_uint64_t operationid, zbx_vector_uint64_t *groupids)
{
	...
	zbx_DBselect_uint64(sql, &parent_groupids);
...
#ifdef HAVE_OPENIPMI
				/* do not forget to update ZBX_IPMI_FIELDS_NUM if number of selected IPMI */
				/* fields changes */
		...
		const ZBX_DB_SERVICE *service, char **params, const char *tz)
{
	...
	result = zbx_DBselect("select name,value from media_type_param where mediatypeid=" ZBX_FS_UI64, mediatypeid);
	while (NULL != (row = zbx_DBfetch(result)))
	{
...
static int	check_operation_conditions(const ZBX_DB_EVENT *event, zbx_uint64_t operationid, unsigned char evaltype)
{
	...
	result = zbx_DBselect("select conditiontype,operator,value"
				" from opconditions"
				" where operationid=" ZBX_FS_UI64
				" order by conditiontype",
			operationid);

	while (NULL != (row = zbx_DBfetch(result)) && 0 == exit)
	{
...
static int	check_escalation(const ZBX_DB_ESCALATION *escalation, const ZBX_DB_ACTION *action,
		const ZBX_DB_EVENT *event, char **error)
{
	DC_ITEM		item;
 ...
 *                                                                            *
 ******************************************************************************/
static void	escalation_execute(ZBX_DB_ESCALATION *escalation, const ZBX_DB_ACTION *action,
		const ZBX_DB_EVENT *event, const ZBX_DB_SERVICE *service, const char *default_timezone,
		zbx_hashset_t *roles)
{
	zabbix_log(LOG_LEVEL_DEBUG, "In %s() escalationid:" ZBX_FS_UI64 " status:%s",
			__func__, escalation->escalationid, zbx_escalation_status_string(escalation->status));
 ...
 *                                                                            *
 ******************************************************************************/
static void	escalation_recover(ZBX_DB_ESCALATION *escalation, const ZBX_DB_ACTION *action,
		const ZBX_DB_EVENT *event, const ZBX_DB_EVENT *r_event, const ZBX_DB_SERVICE *service,
		const char *default_timezone, zbx_hashset_t *roles)
{
	zabbix_log(LOG_LEVEL_DEBUG, "In %s() escalationid:" ZBX_FS_UI64 " status:%s",
			__func__, escalation->escalationid, zbx_escalation_status_string(escalation->status));
		...
		const zbx_vector_uint64_t *service_alarmids)
{
	...
	result = zbx_DBselect("select servicealarmid,clock,value"
			" from service_alarms"
		...
		const char *default_timezone)
{
	...
	result = zbx_DBselect("select escalationid,actionid,triggerid,eventid,r_eventid,nextcheck,esc_step,status,"
