...
static void	hk_check_table_segmentation(const char *table_name, zbx_compress_table_t type)
{
	...
	if (1 == ZBX_DB_TSDB_V1)
	{
		result = zbx_DBselect("select c.attname from _timescaledb_catalog.hypertable_compression c"
				" inner join _timescaledb_catalog.hypertable h on (h.id=c.hypertable_id)"
				" where c.segmentby_column_index<>0 and h.table_name='%s'", table_name);
	}
	else
	{
		result = zbx_DBselect("select attname from timescaledb_information.compression_settings"
	...
	if (1 != i)
	{
		zbx_DBexecute("alter table %s set (timescaledb.compress,timescaledb.compress_segmentby='%s',"
...
static int	hk_get_table_compression_age(const char *table_name)
{
	...
	if (1 == ZBX_DB_TSDB_V1)
	{
		result = zbx_DBselect("select (p.older_than).integer_interval"
...
static void	hk_check_table_compression_age(const char *table_name, int age)
{
	...
	if (age != (compress_after = hk_get_table_compression_age(table_name)))
	{
		...
		if (0 != compress_after)
			DBfree_result(zbx_DBselect("select %s('%s')", COMPRESSION_POLICY_REMOVE, table_name));

		zabbix_log(LOG_LEVEL_DEBUG, "adding compression policy to table: %s age %d", table_name, age);

		res = zbx_DBselect("select %s('%s', integer '%d')", COMPRESSION_POLICY_ADD, table_name, age);
...
static void	hk_history_enable_compression(int age)
{
	...
	{
		DBfree_result(zbx_DBselect("select set_integer_now_func('%s', '"ZBX_TS_UNIX_NOW"', true)",
...
static void	hk_history_disable_compression(void)
{
	...
	for (i = 0; i < (int)ARRSIZE(compression_tables); i++)
	{
		...
		DBfree_result(zbx_DBselect("select %s('%s')", COMPRESSION_POLICY_REMOVE, compression_tables[i].name));
	}
...
void	hk_history_compression_init(void)
{
	...
	if (0 == zbx_strcmp_null(cfg.db.extension, ZBX_CONFIG_DB_EXTENSION_TIMESCALE))
	{
		...
		result = zbx_DBselect("show client_min_messages");

		if (NULL != (row = zbx_DBfetch(result)))
		{
			db_log_level = zbx_strdup(db_log_level, row[0]);
			zbx_DBexecute("set client_min_messages to warning");
		}
		DBfree_result(result);
		...
		if (ON == cfg.db.history_compression_status)
		{
			...
			{
				zbx_DBexecute(ZBX_TS_UNIX_NOW_CREATE);
	...
	if (0 != disable_compression && ZBX_DB_OK > zbx_DBexecute("update config set compression_status=0"))
	...
	{
		zbx_DBexecute("set client_min_messages to %s", db_log_level);
