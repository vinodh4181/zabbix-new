		...
		AGENT_RESULT *result)
{
	...
	if (SUCCEED == zbx_vmware_service_add_perf_counter(service, "HostSystem", hv->id, counterid, instance))
	{
		ret = SYSINFO_RET_OK;
		goto unlock;
	}
		...
		AGENT_RESULT *result)
{
	...
	/* FAIL is returned if counter already exists */
	if (SUCCEED == zbx_vmware_service_add_perf_counter(service, "VirtualMachine", vm->id, counterid, instance))
	{
		ret = SYSINFO_RET_OK;
		goto unlock;
	}
