...
static int	get_value(DC_ITEM *item, AGENT_RESULT *result, zbx_vector_ptr_t *add_results, int config_timeout)
{
	int	res = FAIL;
	...
	switch (item->type)
	{
		...
		case ITEM_TYPE_INTERNAL:
			res = get_value_internal(item, result, config_timeout);
			break;
		case ITEM_TYPE_DB_MONITOR:
#ifdef HAVE_UNIXODBC
			res = get_value_db(item, result, config_timeout);
#else
		...
		case ITEM_TYPE_EXTERNAL:
			...
			res = get_value_external(item, result, config_timeout);
			break;
		case ITEM_TYPE_SSH:
		...
		unsigned char poller_type, int config_timeout)
{
	if (ITEM_TYPE_SNMP == items[0].type)
	{
		...
		get_values_snmp(items, results, errcodes, num, poller_type, config_timeout);
#endif
	}
	...
	else if (1 == num)
	{
		if (SUCCEED == errcodes[0])
			errcodes[0] = get_value(&items[0], &results[0], add_results, config_timeout);
	}
	else
...
static int	get_values(unsigned char poller_type, int *nextcheck, int config_timeout)
{
	...
	zbx_check_items(items, errcodes, num, results, &add_results, poller_type, config_timeout);
