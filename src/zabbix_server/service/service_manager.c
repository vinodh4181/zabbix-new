...
static void	db_get_events(zbx_hashset_t *problem_events)
{
	...
	result = zbx_DBselect("select p.eventid,p.clock,p.severity,t.tag,t.value,p.ns"
			" from problem p"
...
static void	sync_service_problem_tags(zbx_service_manager_t *service_manager, int *updated, int revision)
{
	...
	result = zbx_DBselect("select service_problem_tagid,serviceid,tag,operator,value"
...
static void	sync_services(zbx_service_manager_t *service_manager, int *updated, int revision)
{
	...
	result = zbx_DBselect("select serviceid,status,algorithm,name,weight,propagation_rule,propagation_value"
			" from services");
...
static void	sync_service_rules(zbx_service_manager_t *service_manager, int *updated, int revision)
{
	...
	result = zbx_DBselect("select service_status_ruleid,serviceid,type,limit_value,limit_status,new_status"
...
static void	sync_service_tags(zbx_service_manager_t *service_manager, int revision)
{
	...
	result = zbx_DBselect("select servicetagid,serviceid,tag,value from service_tag");
...
static void	sync_services_links(zbx_service_manager_t *service_manager, int *updated, int revision)
{
	...
	result = zbx_DBselect("select linkid,serviceupid,servicedownid from services_links");
...
static void	sync_service_problems(zbx_hashset_t *services, zbx_hashset_t *service_problems_index)
{
	...
	result = zbx_DBselect("select service_problemid,eventid,serviceid,severity from service_problem");
...
static void	sync_actions(zbx_service_manager_t *service_manager, int revision)
{
	...
	result = zbx_DBselect("select actionid,evaltype,formula from actions "
				"where eventsource=%d"
					" and status=%d",
...
static void	sync_action_conditions(zbx_service_manager_t *service_manager, int revision)
{
	...
	result = zbx_DBselect("select c.conditionid,c.actionid,c.conditiontype,c.operator,c.value,c.value2"
...
static void	sync_config(zbx_service_manager_t *service_manager)
{
	...
	result = zbx_DBselect("select severity_name_0,severity_name_1,severity_name_2,severity_name_3,severity_name_4,"
...
static void	db_update_services(zbx_service_manager_t *manager)
{
	...
	while (ZBX_DB_DOWN == zbx_DBcommit());
...
static int	db_update_service_problems(const zbx_vector_ptr_t *event_severities)
{
	...
	while (ZBX_DB_DOWN == (txn_rc = zbx_DBcommit()));
