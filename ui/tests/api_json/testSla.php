		...
		return [
			...
			'Full sla' => [
				'sla' => [
					...
					'description' => 'Обновление почтового сервера',
					'service_tags' => [
						[
							'tag' => 'tag',
							'operator' => ZBX_SLA_SERVICE_TAG_OPERATOR_LIKE,
	...
	public static function sla_get_data_invalid(): array {
		...
		return [
			...
			'Filter period negative int'  => [
				...
				'expected' => [
					'error' => 'Invalid parameter "/filter/period/1": value must be one of '.implode(', ', [
						ZBX_SLA_PERIOD_DAILY,
						ZBX_SLA_PERIOD_WEEKLY,
						ZBX_SLA_PERIOD_MONTHLY,
						ZBX_SLA_PERIOD_QUARTERLY, ZBX_SLA_PERIOD_ANNUALLY]
					).'.'
				]
			],
			'Filter period float'  => [
			...
			'Filter slo negative' => [
				...
				'expected' => [
					'error' => 'Invalid parameter "/filter/slo/1": value must be within the range of 0-100.'
				]
			],
			'Filter slo non-array out of range' => [
				'request' => [
					'output' => [],
					'filter' => [
						'slo' => 999
			...
			'Filter effective_date empty/string' => [
				'request' => [
					'output' => [],
					'filter' => [
						'effective_date' => ''
			...
			'Filter effective_date negative int' => [
				'request' => [
					'output' => [],
					'filter' => [
						'effective_date' => -1
				...
				'expected' => [
					'error' => 'Invalid parameter "/filter/effective_date/1": value must be one of 0-'.ZBX_MAX_DATE.'.'
				]
			],
			'Filter effective_date float' => [
				'request' => [
					'output' => [],
					'filter' => [
						'effective_date' => 1.1
			...
			'Filter effective_date out of scope' => [
				'request' => [
					'output' => [],
					'filter' => [
						'effective_date' => [ZBX_MAX_DATE + 1]
				...
				'expected' => [
					'error' => 'Invalid parameter "/filter/effective_date/1": a number is too large.'
				]
			],
			...
			'Filter timezone empty/string' => [
				...
				'expected' => [
					'error' => 'Invalid parameter "/filter/timezone/1": value must be one of '.$timezone_list.'.'
				]
			],
			'Filter timezone negative int' => [
				'request' => [
					'output' => [],
					'filter' => [
						'timezone' => -1
			...
			'Filter status negative int' => [
				...
				'expected' => [
					'error' => 'Invalid parameter "/filter/status/1": value must be one of '.implode(', ', [
						ZBX_SLA_STATUS_DISABLED, ZBX_SLA_STATUS_ENABLED
					]).'.'
				]
			],
			'Filter status float' => [
			...
			'Search description negative int' => [
				'request' => [
					'output' => [],
					'search' => [
						'description' => -1
